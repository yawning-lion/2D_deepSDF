...
def run_dichotomy_loop(pin_batch, shape, latent_code, nn, epoch = 10):
    latent = latent_code[shape]
...
def Monte_Carlo_getseeds(num_pin, pin_length, shape, mode, latent_code, nn):
    pin_batch = get_pin(num_pin, pin_length)
    seeds = run_dichotomy_loop(pin_batch, shape, latent_code, nn)
    return seeds, shape, mode


batch_Monte_Carlo_getseeds = vmap(Monte_Carlo_getseeds, in_axes = (None, None, 0, None, None, None), out_axes = 0)
...
def run_get_seeds_loop(num_seeds, num_pin, pin_length, mode, index):
    ...
    for i in range(index):
        seeds, shape, mode = Monte_Carlo_getseeds(num_pin, pin_length, i, mode, latent_code, nn)
